...
def standardise(input_dims: np.array, X: np.array) -> np.array:
    ...
    for i in range(4):
        ...
        stdhi_cond = np.subtract(hi_array.astype(float), array_mean_val.astype(float))
        stdhi_cond = np.divide(stdhi_cond.astype(float), stdev.astype(float))
        std_hi[:,i] = stdhi_cond
        stdlo_cond = np.subtract(lo_array.astype(float), array_mean_val.astype(float))
    ...
    return(np.array(std_dims),np.array((std_lo, std_hi)),rev_data_std)
...
def update_row(M1: np.array, M2: np.array, X: np.array, I: float, col_row: int):
    ...
    if col_row == 0:
        ...
        print("B matrix column ", I, "upper bounds", X_high)      
...
s, CSs, CLM, PAs, bounds_vec, rev_stdise = problem_setup(data, bounds_vec, 1)
print("bounds_vec ", bounds_vec)

#Passing to function that carries out BPCA (as in Chemometrics 2007; 21: 547–556
#DOI: 10.1002/cem)
A, B = solve_for_A_B(CSs, CLM, bounds_vec, n)
partial = np.matmul(A, B.T)
print("partial temp", partial[3])
print(rev_stdise[1])
print(rev_stdise[0])
reconst = (partial*rev_stdise[1])+rev_stdise[0]
print("reconstructed from 1/4 dimensions ")
print(reconst)
print("original 4D")
print(data)