...
class ExodusReader():
    def __init__(self, file_name):
        ...
        self.__scaler = preprocessing.StandardScaler().fit(positions)
        scaled_pos = self.__scaler.transform(positions)
        self.__interpolater = LinearNDInterpolator(scaled_pos, temperatures)


    def get_point_temp(self, pos):
        scaled_pos_xy = self.__scaler.transform(pos.reshape(1, 3))
        ...
        for i in tqdm(range(len(x_values))):
            for j in range(len(y_values)):
                if self.check_face(x_values[i], y_values[j]):
                    rounded_z = np.round(x_values[i], 7)
                    rounded_y = np.round(y_values[j], 7)
                    temp.append(self.get_point_temp(np.array([0.01, rounded_y, rounded_z])))
                    x.append(rounded_z)
                    y.append(rounded_y)
        print(temp)
        return x, y, temp
